...
void D_DoAdvanceDemo(void)
{
    ...
    if (bfgedition && !doomgeneric_strcasecmp(pagename, "TITLEPIC") &&
...
static char *GetGameName(char *gamename)
{
    ...
    for (i = 0; i < arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            // Has been replaced.
            // We need to expand via doomgeneric_printf to include the Doom
            // version number We also need to cut off spaces to get the basic
            // name
...
static void SetMissionForPackName(char *pack_name)
{
    ...
    {
        if (!doomgeneric_strcasecmp(pack_name, packs[i].name))
        {
...
void D_IdentifyVersion(void)
{
    ...
    if (gamemission == none)
    {
        ...
        {
            if (!doomgeneric_strncasecmp(lumpinfo[i].name, "MAP01", 8))
            {
                gamemission = doom2;
                break;
            }
            else if (!doomgeneric_strncasecmp(lumpinfo[i].name, "E1M1", 8))
            {
                gamemission = doom;
                break;
            }
