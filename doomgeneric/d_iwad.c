...
static char *CheckDirectoryHasIWAD(char *dir, char *iwadname)
{
    ...
    if (!strcmp(dir, "."))
    {
        filename = doomgeneric_strdup(iwadname);
    }
    else
    {
    ...
    doomgeneric_printf("Trying IWAD file:%s\n", filename);
...
static void AddDoomWadPath(void)
{
    ...
    doomwadpath = doomgeneric_strdup(doomwadpath);
...
char *D_FindWADByName(char *name)
{
    ...
    for (i=0; i<num_iwad_dirs; ++i)
    {
        ...
        {
            return doomgeneric_strdup(iwad_dirs[i]);
        }
...
char *D_FindIWAD(int mask, GameMission_t *mission)
{
    ...
    else
    {
        ...
        doomgeneric_printf("-iwad not specified, trying a few iwad names\n");
